...
implements IChunkProvider {
    ...
    public ChunkProviderOreSpawn5(World par1World, long par2, boolean par4) {
        ...
        this.field_147435_p = par1World.func_72912_H().func_76067_t();
    ...
    public void func_147424_a(int p_147424_1_, int p_147424_2_, Block[] p_147424_3_) {
        int b0 = 63;
        this.biomesForGeneration = this.worldObj.func_72959_q().func_76937_a(this.biomesForGeneration, p_147424_1_ * 4 - 2, p_147424_2_ * 4 - 2, 10, 10);
                    ...
                    for (int l2 = 0; l2 < 8; ++l2) {
                        ...
                        for (int i3 = 0; i3 < 4; ++i3) {
                            ...
                            for (int k3 = 0; k3 < 4; ++k3) {
                                double d;
                                d15 += d16;
                                p_147424_3_[j3 += short1] = d > 0.0 ? OreSpawnMain.CrystalStone : (k2 * 8 + l2 < b0 ? Blocks.field_150355_j : null);
                            }
                            d10 += d12;
                            d11 += d13;
                        }
    ...
    private void func_147423_a(int p_147423_1_, int p_147423_2_, int p_147423_3_) {
        ...
        double d2 = 512.0;
        double d3 = 512.0;
        this.field_147426_g = this.noiseGen6.func_76305_a(this.field_147426_g, p_147423_1_, p_147423_3_, 5, 5, 200.0, 200.0, 0.5);
        this.field_147427_d = this.field_147429_l.func_76304_a(this.field_147427_d, p_147423_1_, p_147423_2_, p_147423_3_, 5, 33, 5, 8.555150000000001, 4.277575000000001, 8.555150000000001);
        this.field_147428_e = this.field_147431_j.func_76304_a(this.field_147428_e, p_147423_1_, p_147423_2_, p_147423_3_, 5, 33, 5, 684.412, 684.412, 684.412);
        this.field_147425_f = this.field_147432_k.func_76304_a(this.field_147425_f, p_147423_1_, p_147423_2_, p_147423_3_, 5, 33, 5, 684.412, 684.412, 684.412);
        boolean flag1 = false;
        boolean flag = false;
        int l = 0;
        int i1 = 0;
    ...
    public void addCrystalTrees(World world, Random random, int chunkX, int chunkZ, Chunk chunk) {
        ...
        block0: for (int i = 0; i < howmany; ++i) {
            ...
            for (int posY = 128; posY > 40; --posY) {
                if (OreSpawnMain.getBlockIDInChunk(chunk, posX, posY, posZ) != Blocks.field_150350_a || OreSpawnMain.getBlockIDInChunk(chunk, posX, posY - 1, posZ) != OreSpawnMain.CrystalGrass) continue;
                if (what == 0) {
    ...
    public void makeScragglyCrystalBranch(World world, int x, int y, int z, int len, int biasx, int biasz, Chunk chunk) {
        for (int k = 0; k < len; ++k) {
            ...
            if ((bid = OreSpawnMain.getBlockIDInChunk(chunk, x += ix, y += (iy = world.field_73012_v.nextInt(3) > 0 ? 1 : 0), z += iz)) != Blocks.field_150350_a && bid != OreSpawnMain.MyCrystalTreeLog && bid != OreSpawnMain.MyCrystalLeaves2) {
                return;
            }
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, x, y, z, OreSpawnMain.MyCrystalTreeLog, 0);
            for (int m = -1; m < 2; ++m) {
                for (int n = -1; n < 2; ++n) {
                    if (world.field_73012_v.nextInt(2) != 1 || (bid = OreSpawnMain.getBlockIDInChunk(chunk, x + m, y, z + n)) != Blocks.field_150350_a) continue;
                    OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, x + m, y, z + n, OreSpawnMain.MyCrystalLeaves2, 0);
                }
            }
            if (world.field_73012_v.nextInt(2) != 1 || (bid = OreSpawnMain.getBlockIDInChunk(chunk, x, y + 1, z)) != Blocks.field_150350_a) continue;
    ...
    public void ScragglyCrystalTreeWithBranches(World world, int x, int y, int z, Chunk chunk) {
        ...
        for (k = i; k < j; ++k) {
            ...
            if (world.field_73012_v.nextInt(4) == 1) {
                this.makeScragglyCrystalBranch(world, x, y, z, world.field_73012_v.nextInt(1 + j - k), world.field_73012_v.nextInt(2) - world.field_73012_v.nextInt(2), world.field_73012_v.nextInt(2) - world.field_73012_v.nextInt(2), chunk);
            }
            for (int m = -1; m < 2; ++m) {
                for (int n = -1; n < 2; ++n) {
                    if (world.field_73012_v.nextInt(2) != 1 || (bid = OreSpawnMain.getBlockIDInChunk(chunk, x + m, y, z + n)) != Blocks.field_150350_a) continue;
    ...
    public void TallCrystalTree(World world, int x, int y, int z, Chunk chunk) {
        ...
        int j = i + world.field_73012_v.nextInt(18 - OreSpawnMain.LessLag * 2);
        for (k = 0; k < i; ++k) {
            bid = OreSpawnMain.getBlockIDInChunk(chunk, x, y + k, z);
        ...
        y += i - 1;
        for (k = i; k < j && ((bid = OreSpawnMain.getBlockIDInChunk(chunk, x, ++y, z)) == Blocks.field_150350_a || bid == OreSpawnMain.MyCrystalTreeLog || bid == OreSpawnMain.MyCrystalLeaves); ++k) {
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, x, y, z, OreSpawnMain.MyCrystalTreeLog, 0);
            if (k % 4 != 0) continue;
            for (m = -1; m < 2; ++m) {
                for (n = -1; n < 2; ++n) {
    ...
    public boolean generateOre(World par1World, Random par2Random, int par3, int par4, int par5, Chunk chunk, Block newbid, int numberOfBlocks, Block oldbid) {
        ...
        double d0 = (float)(par3 + 8) + MathHelper.func_76126_a((float)f) * (float)numberOfBlocks / 8.0f;
        double d1 = (float)(par3 + 8) - MathHelper.func_76126_a((float)f) * (float)numberOfBlocks / 8.0f;
        double d2 = (float)(par5 + 8) + MathHelper.func_76134_b((float)f) * (float)numberOfBlocks / 8.0f;
        double d3 = (float)(par5 + 8) - MathHelper.func_76134_b((float)f) * (float)numberOfBlocks / 8.0f;
