...
public class CrystalMaze {
    ...
    public void buildCrystalMaze(World world, int x, int y, int z, Chunk chunk) {
        for (int i = 0; i < 16; ++i) {
            for (int j = 0; j < 16; ++j) {
                for (int k = 0; k < 3; ++k) {
                    OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, x + j, y + k, z + i, Blocks.field_150350_a, 0);
                }
            }
        }
    ...
    private void openCrystalMaze(World world, int xx, int yy, int zz, int xw, int zw, int csz, Chunk chunk) {
        ...
        int i;
        for (i = 0; i < zw * csz; ++i) {
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx, yy, zz + i, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx, yy + 1, zz + i, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx, yy + 2, zz + i, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + i, yy, zz, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + i, yy + 1, zz, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + i, yy + 2, zz, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + zw * csz - 1, yy, zz + i, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + zw * csz - 1, yy + 1, zz + i, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + zw * csz - 1, yy + 2, zz + i, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + i, yy, zz + zw * csz - 1, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + i, yy + 1, zz + zw * csz - 1, Blocks.field_150350_a, 0);
            OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + i, yy + 2, zz + zw * csz - 1, Blocks.field_150350_a, 0);
        }
        for (i = 0; i < zw * csz; ++i) {
            for (j = 0; j < zw * csz; ++j) {
                OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + j, yy - 1, zz + i, Blocks.field_150357_h, 0);
                OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, xx + j, yy + 3, zz + i, Blocks.field_150357_h, 0);
            }
        }
        for (int k = 0; k < 4; ++k) {
            i = world.field_73012_v.nextInt(zw * csz);
            j = world.field_73012_v.nextInt(zw * csz);
    ...
    private void clearArea(World world, int x, int y, int z, Chunk chunk) {
        ...
        for (i = 0; i < 60; ++i) {
            ...
            for (j = 0; j < hi; ++j) {
                for (k = 0; k < 30; ++k) {
                    OreSpawnMain.setBlockIDWithMetadataInChunk(chunk, x + i, y + j, z + k, Blocks.field_150350_a, 0);
                }
            }
        }
        for (i = 0; i < 5; ++i) {
